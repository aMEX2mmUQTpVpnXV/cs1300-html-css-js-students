...
const updatePage = async () => {
  ...
  proteinFruitArray = fruitsArray.filter((item) => {
    return item.nutritions.protein > 1;
  });
  proteinFruitArray = proteinFruitArray.map((item) => {
    return item.name;
  });
  caloriesFruitArray = fruitsArray.filter((item) => {
    return item.nutritions.calories < 50;
  });
  caloriesFruitArray = caloriesFruitArray.map((item) => {
    return item.name;
  });
  if (count === 0) {
    addFact("Here is a list of high protein fruits: \n\n " + proteinFruitArray);
  }
  if (count === 1) {
    addFact("Here is a list of low calorie fruits: \n\n " + caloriesFruitArray);
  }
  count = count + 1;
};

const addFact = (msg, name, array) => {
  ...
  const existingElement = document.getElementById("fact-gallery");
